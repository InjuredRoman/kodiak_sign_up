...
class EnrollmentUpdate extends React.Component {
        ...
        retrieve_enrollments_by_token(
            ...
            response => {
                this.setState(
                    {
                        successful: true,
                        enrollments: response['enrollments'].filter(e => {
    ...
    confirm(enrollment_info) {
        ...
        update_enrollment(
            ...
            response => {
                this.setState(
                    { successful: true, response: response, loaded: true },
                    () => {
                        this.props.enqueueSnackbar(
                            enrollment_info.first_name +
                                ' has been successfully enrolled into ' +
                                enrollment_info.activity_title,
                            { variant: confirm_variant }
                        );
                        this.setState({
                            enrollments: this.state.enrollments.filter(e => {
                                return e.id !== e_id;
                            }),
                        });
                    }
            ...
            response => {
                this.setState(
                    ...
                    () => {
                        this.props.enqueueSnackbar(
                            enrollment_info.first_name +
                                ' no longer has a pending enrollment in ' +
                                enrollment_info.activity_title,
